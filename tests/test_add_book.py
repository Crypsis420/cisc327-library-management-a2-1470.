...
def test_add_book_invalid_isbn_non_digit():
    success, msg = add_book_to_catalog("Broncos Are Awesome", "Sean Peyton", "12345abc90123", 5)
    assert success is False
    assert "isbn must be digits" in msg.lower()

def test_add_book_title_too_long():
    long_title = "A" * 201
    success, msg = add_book_to_catalog(long_title, "LONG", "1234567890123", 5)
    assert success is False
    assert "title must be less than 200 characters" in msg.lower()

def test_add_book_author_too_long():
    long_author = "B" * 101
    success, msg = add_book_to_catalog("Hello", long_author, "1234567890123", 5)
    assert success is False
